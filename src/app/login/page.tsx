...
import { Film, Lock, User, Loader2 } from 'lucide-react';
...
function LoginContent() {
  ...
  const getErrorMessage = (error: string) => {
    switch (error) {
      ...
      case 'CredentialsSignin':
        return 'Invalid username or password, or account locked.';
      default:
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-[#020205] overflow-hidden py-12 px-4">
      ...
      {/* Login Card */}
      <div className="w-full max-w-md p-8 rounded-3xl glass-panel relative z-10 border-white/5 shadow-2xl">
        ...
        {(error || localError) && (
          <div className="mb-6 p-4 rounded-xl bg-red-950/40 border border-red-500/30 flex items-start gap-3 animate-shake">
            <p className="text-sm text-red-200 font-medium">{localError || getErrorMessage(error as string)}</p>
          </div>
        )}
        ...
        <form onSubmit={handleCredentialsLogin} className="space-y-4 mb-6">
          <div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3.5 px-4 bg-gradient-to-r from-purple-600 to-amber-600 hover:from-purple-500 hover:to-amber-500 text-white rounded-xl font-bold transition-all active:scale-[0.98] shadow-lg disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center gap-2"
          >
        ...
        <div className="relative flex items-center py-2 mb-6">
          ...
          <span className="flex-shrink-0 mx-4 text-xs text-slate-500 uppercase tracking-widest font-semibold">Or continue with</span>
          <div className="flex-grow border-t border-white/10"></div>
        </div>
